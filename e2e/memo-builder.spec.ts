...
test("labels split development and activity tables as continuations in preview and DOCX", async ({ page }) => {
  ...
  for (const [title, headers, bodyMarker, expectedGrid, borderRange] of [
    ...
    const layoutCells = physicalRows
      .flat()
      .filter((cell) => cell.end <= borderColumnStart || cell.start >= borderColumnEnd);
    expect(layoutCells.length).toBeGreaterThan(0);
    for (const cell of layoutCells) expect(cell.edges.size).toBe(0);
    expect(
      physicalRows
        .flat()
        .filter((cell) => cell.start < borderColumnEnd && cell.end > borderColumnStart)
...
test("activity and appendix dates share individual grouping, punctuation, month, and year rules", async ({ page }) => {
  ...
  const cases = [
    ...
    {
      id: "activity-date-cross-month-single-conjunction",
      dates: [
        "2026-08-05",
        "2026-08-06",
        "2026-08-07",
      ...
      expected: "5-7, 15, 20, 28 Agustus, dan 4 September 2026",
    },
    {
